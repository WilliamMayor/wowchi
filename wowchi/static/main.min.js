...
if(wowchi.only_incomplete()){keep&=!achi.is_completed();}
if(wowchi.only_reward()){keep&=!!achi.reward;}
return keep;});});self.categories=_.map(details.categories,function(c){return new Category(c,self);});self.explore=ko.observable();self.is_opened=ko.observable(false);self.scroll_to=ko.observable(false);self.expand=function(){self.is_opened(!self.is_opened());};self.afterRenderExplore=function(elements){if(self.scroll_to()){$(elements).parents("li").scrollintoview();self.scroll_to(false);}};self.open_recursively=function(){self.is_opened(true);if(self.parent){self.parent.open_recursively();}};}
function Wowchi(){var self=this;self.working=ko.observable(0);self.more_work=function(){self.working(self.working()+1);};self.less_work=function(){self.working(self.working()-1);};self.locales=locales;self.races=races;self.classes=classes;self.locale=ko.observable();self.realms=ko.observableArray();self.realm=ko.observable();self.character_name=ko.observable();self.character_class=ko.observable(7);self.character_race=ko.observable(2);self.search_query=ko.observable("");self.only_incomplete=ko.observable(false);self.only_reward=ko.observable(false);self.completed=ko.observableArray();self.criteria=ko.observableArray();self.available=ko.observableArray();self.queue=ko.observableArray();self.explore=ko.observable();self.character_class_faction=ko.pureComputed(function(){var race_id=self.character_race();var class_faction="";var race=_.find(self.races,function(possible){return possible.id===race_id;});if(race){class_faction=race.side+" ";}

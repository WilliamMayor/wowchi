...
function Locale(country,locale,host){var self=this;self.country=country;self.locale=locale;self.host=host;}
var locales=[new Locale('USA','en_US','http://us.battle.net'),new Locale('Mexico','es_MX','http://us.battle.net'),new Locale('Brazil','pt_BR','http://us.battle.net'),new Locale('UK','en_GB','http://eu.battle.net'),new Locale('Spain','es_ES','http://eu.battle.net'),new Locale('France','fr_FR','http://eu.battle.net'),new Locale('Russia','ru_RU','http://eu.battle.net'),new Locale('Germany','de_DE','http://eu.battle.net'),new Locale('Portugal','pt_PT','http://eu.battle.net'),new Locale('Italy','it_IT','http://eu.battle.net'),new Locale('Korea','ko_KR','http://kr.battle.net'),new Locale('Taiwan','zh_TW','http://tw.battle.net'),new Locale('China','zh_CN','http://www.battlenet.com.cn')];var races=[{id:1,mask:1,side:"alliance",name:"Human"},{id:1,mask:1,side:"alliance",name:"Human"},{id:2,mask:2,side:"horde",name:"Orc"},{id:2,mask:2,side:"horde",name:"Orc"},{id:3,mask:4,side:"alliance",name:"Dwarf"},{id:3,mask:4,side:"alliance",name:"Dwarf"},{id:4,mask:8,side:"alliance",name:"Night Elf"},{id:4,mask:8,side:"alliance",name:"Night Elf"},{id:5,mask:16,side:"horde",name:"Undead"},{id:5,mask:16,side:"horde",name:"Undead"},{id:6,mask:32,side:"horde",name:"Tauren"},{id:6,mask:32,side:"horde",name:"Tauren"},{id:7,mask:64,side:"alliance",name:"Gnome"},{id:7,mask:64,side:"alliance",name:"Gnome"},{id:8,mask:128,side:"horde",name:"Troll"},{id:8,mask:128,side:"horde",name:"Troll"},{id:9,mask:256,side:"horde",name:"Goblin"},{id:10,mask:512,side:"horde",name:"Blood Elf"},{id:11,mask:1024,side:"alliance",name:"Draenei"},{id:11,mask:1024,side:"alliance",name:"Draenei"},{id:22,mask:2097152,side:"alliance",name:"Worgen"},{id:24,mask:8388608,side:"neutral",name:"Pandaren"},{id:25,mask:16777216,side:"alliance",name:"Pandaren"},{id:26,mask:33554432,side:"horde",name:"Pandaren"}];var classes=[{id:3,mask:4,powerType:"focus",name:"Hunter"},{id:4,mask:8,powerType:"energy",name:"Rogue"},{id:1,mask:1,powerType:"rage",name:"Warrior"},{id:2,mask:2,powerType:"mana",name:"Paladin"},{id:7,mask:64,powerType:"mana",name:"Shaman"},{id:8,mask:128,powerType:"mana",name:"Mage"},{id:5,mask:16,powerType:"mana",name:"Priest"},{id:6,mask:32,powerType:"runic-power",name:"Death Knight"},{id:11,mask:1024,powerType:"mana",name:"Druid"},{id:9,mask:256,powerType:"mana",name:"Warlock"},{id:10,mask:512,powerType:"energy",name:"Monk"}];var message={template_error:_.template("<li class='error'><%= message %><a class='close' href='javascript:void(0)'><i class='fa fa-times fa-fw'></i></a></li>"),template_warning:_.template("<li class='warning'><%= message %><a class='close' href='javascript:void(0)'><i class='fa fa-times fa-fw'></i></a></li>"),error:function(msg){Rollbar.error(msg);$("#messages").append(message.template_error({message:msg}));},warning:function(msg){Rollbar.warning(msg);$("#messages").append(message.template_warning({message:msg}));},init:function(){$("#messages").on('click','li a.close',function(){$(this).parents("li").remove();});}};function Realm(name){var self=this;self.name=name;}
function Criterion(details){var self=this;self.details=details;self.id=details.id;self.quantity=details.quantity;self.description=details.description;self.max=details.max;self.is_completed=ko.observable(false);}
function Achievement(category,details){var self=this;self.category=category;self.details=details;self.id=details.id;self.title=details.title;self.description=details.description;self.icon_18="http://media.blizzard.com/wow/icons/18/"+details.icon+".jpg";self.icon_36="http://media.blizzard.com/wow/icons/36/"+details.icon+".jpg";self.icon_56="http://media.blizzard.com/wow/icons/56/"+details.icon+".jpg";self.wowhead="http://www.wowhead.com/achievement="+self.id;self.reward=details.reward;self.accountWide=details.accountWide;self.criteria=_.map(details.criteria,function(c){return new Criterion(c);});self.pct_criteria_complete=ko.pureComputed(function(){var max=self.criteria.length;var actual=_.reduce(self.criteria,function(memo,c){if(c.is_completed()){memo++;}
return memo;},0);return actual/max;});self.is_completed=ko.observable(false);self.is_exploring=ko.pureComputed(function(){return self.category.explore()&&self.category.explore().id===self.id;});self.explore=function(){if(self.is_exploring()){self.category.explore(null);}else{self.category.explore(self);}};self.explore_in_queue=function(){self.category.scroll_to(true);self.category.open_recursively();self.category.explore(self);};self.remove_from_queue=function(){wowchi.queue.remove(self);if(wowchi.explore()&&wowchi.explore().id===self.id){wowchi.explore(null);}};self.add_to_queue=function(){if(self.can_add_to_queue()){wowchi.queue.push(self);}};self.can_add_to_queue=ko.pureComputed(function(){return!self.is_completed()&&self.is_not_in_queue();});self.is_not_in_queue=ko.pureComputed(function(){return wowchi.queue.indexOf(self)===-1;});}
function Category(details,parent){var self=this;self.details=details;self.parent=parent;self.id=details.id;self.name=details.name;self.all_achievements=_.map(details.achievements,function(a){return new Achievement(self,a);});self.achievements=ko.pureComputed(function(){var q=wowchi.search_query().toLowerCase();return ko.utils.arrayFilter(self.all_achievements,function(achi){var keep=true;if(!!q){keep&=achi.title.toLowerCase().indexOf(q)!==-1;}
switch(wowchi.filter_status()){case"incomplete":keep&=!achi.is_completed();break;case"complete":keep&=achi.is_completed();break;case"nearly":keep&=!achi.is_completed()&&achi.pct_criteria_complete()>=0.8&&achi.pct_criteria_complete()<1;break;}
switch(wowchi.filter_reward()){case"has":keep&=!!achi.reward;break;case"no":keep&=!achi.reward;break;case"title":keep&=!!achi.reward&&(achi.reward.indexOf("Title Reward: ")!==-1||achi.reward.indexOf("Title: ")!==-1);break;case"blueprint":keep&=!!achi.reward&&achi.reward.toLowerCase().indexOf("blueprint")!==-1;break;case"other":keep&=!!achi.reward&&achi.reward.indexOf("Title Reward: ")===-1&&achi.reward.indexOf("Title: ")===-1&&achi.reward.toLowerCase().indexOf("blueprint")===-1;break;}
return keep;});});self.categories=_.map(details.categories,function(c){return new Category(c,self);});self.explore=ko.observable();self.is_opened=ko.observable(false);self.scroll_to=ko.observable(false);self.meets_filter=ko.pureComputed(function(){return self.achievements().length>0||_.reduce(self.categories,function(memo,c){return memo||c.meets_filter();},false);});self.expand=function(){self.is_opened(!self.is_opened());};self.afterRenderExplore=function(elements){if(self.scroll_to()){$(elements).parents("li").scrollintoview();self.scroll_to(false);}};self.open_recursively=function(){self.is_opened(true);if(self.parent){self.parent.open_recursively();}};}
function Wowchi(){var self=this;self.working=ko.observable(0);self.more_work=function(){self.working(self.working()+1);};self.less_work=function(){self.working(self.working()-1);};self.locales=locales;self.races=races;self.classes=classes;self.locale=ko.observable();self.realms=ko.observableArray();self.realm=ko.observable();self.character_name=ko.observable();self.character_class=ko.observable(7);self.character_race=ko.observable(2);self.search_query=ko.observable("");self.filter_status=ko.observable("any");self.filter_reward=ko.observable("any");self.completed=ko.observableArray();self.criteria=ko.observableArray();self.available=ko.observableArray();self.queue=ko.observableArray();self.explore=ko.observable();self.character_class_faction=ko.pureComputed(function(){var race_id=self.character_race();var class_faction="";var race=_.find(self.races,function(possible){return possible.id===race_id;});if(race){class_faction=race.side+" ";}
var class_id=self.character_class();var _class=_.find(self.classes,function(possible){return possible.id===class_id;});if(_class){class_faction+=_class.name.toLowerCase();}
return class_faction;});self.find_realms=function(){var l=self.locale();if(!l){return;}
self.more_work();$.ajax({url:self.locale().host+"/api/wow/realm/status?jsonp=?&locale="+self.locale().locale,dataType:"jsonp",jsonp:"jsonp",jsonpCallback:"jsonpCallback"}).done(function(data,textStatus,jqXHR){self.realms(_.map(data.realms,function(r){return new Realm(r.name);}));}).fail(function(jqXHR,textStatus,errorThrown){console.error(jqXHR,textStatus,errorThrown);message.error("There was an error fetching realm data, please try again");}).always(function(data_jqXHR,textStatus,jqXHR_errorThrown){self.less_work();});};self.find_available=function(){var l=self.locale();var r=self.realm();var n=self.character_name();if(!(l&&r&&n)){return;}
Rollbar.configure({payload:{person:{id:l.locale+":"+r.name+":"+n}}});Rollbar.info("User finding data");self.more_work();$.ajax({url:l.host+"/api/wow/data/character/achievements?jsonp=?&locale="+l.locale,dataType:"jsonp",jsonp:"jsonp",jsonpCallback:"jsonpCallback"}).done(function(data,textStatus,jqXHR){self.available(_.map(data.achievements,function(a){return new Category(a);}));self.find_completed();}).fail(function(jqXHR,textStatus,errorThrown){console.error(jqXHR,textStatus,errorThrown);message.error("There was an error fetching global achivement data, please try again");}).always(function(){self.less_work();});};self.find_completed=function(){var l=self.locale();var r=self.realm();var n=self.character_name();if(!(l&&r&&n)){return;}
self.more_work();$.ajax({url:l.host+"/api/wow/character/"+r.name+"/"+n+"?jsonp=?&fields=achievements&locale="+l.locale,dataType:"jsonp",jsonp:"jsonp",jsonpCallback:"jsonpCallback"}).done(function(data,textStatus,jqXHR){self.character_class(data.class);self.character_race(data.race);self.completed(data.achievements.achievementsCompleted);self.criteria(data.achievements.criteria);}).fail(function(jqXHR,textStatus,errorThrown){console.error(jqXHR,textStatus,errorThrown);message.error("There was an error fetching character achivement data, please try again");}).always(function(){self.less_work();});};self.find_achievement=function(aid,categories){if(!categories){categories=self.available();}
var a;_.find(categories,function(category){a=_.find(category.all_achievements,function(achievement){return achievement.id===aid;});if(!a&&category.categories){a=self.find_achievement(aid,category.categories);}
if(a){return true;}});return a;};self.find_criterion=function(cid,categories){if(!categories){categories=self.available();}
var c;_.find(categories,function(category){_.find(category.all_achievements,function(achievement){c=_.find(achievement.criteria,function(criterion){return criterion.id===cid;});if(c){return true;}});if(!c&&category.categories){c=self.find_criterion(cid,category.categories);}
if(c){return true;}});return c;};self.queue_explore=function(achievement){if(self.explore()&&self.explore().id===achievement.id){self.explore(null);}else{self.explore(achievement);}};self.update_complete=function(){_.each(self.completed(),function(aid){var a=self.find_achievement(aid);if(a){a.is_completed(true);}});};self.update_criteria=function(){_.each(self.criteria(),function(cid){var c=self.find_criterion(cid);if(c){c.is_completed(true);}});};self.load=function(){if('localStorage'in window&&window.localStorage!==null){var locale=window.localStorage.locale;if(locale){var l=_.find(self.locales,function(l){return l.country===locale;});if(l){self.locale(l);}}
var realms=window.localStorage.realms;if(realms){self.realms(_.map(JSON.parse(realms),function(r){return new Realm(r.name);}));}
var realm=window.localStorage.realm;if(realm){var r=_.find(self.realms(),function(r){return r.name===realm;});if(r){self.realm(r);}}
var character_name=window.localStorage.character_name;if(character_name){self.character_name(character_name);}
var character_race=window.localStorage.character_race;if(character_race){self.character_race(JSON.parse(character_race));}
var character_class=window.localStorage.character_class;if(character_class){self.character_class(JSON.parse(character_class));}
var available=window.localStorage.available;if(available){self.available(_.map(JSON.parse(available),function(a){return new Category(a);}));}
var completed=window.localStorage.completed;if(completed){self.completed(JSON.parse(completed));self.update_complete();}
var criteria=window.localStorage.criteria;if(criteria){self.criteria(JSON.parse(criteria));self.update_criteria();}
var queue=window.localStorage.queue;if(queue){self.queue(_.map(JSON.parse(queue),function(aid){return self.find_achievement(aid);}));}}
self.init_listeners();};self.init_listeners=function(){if('localStorage'in window&&window.localStorage!==null){self.locale.subscribe(function(){window.localStorage.locale=self.locale().country;});self.realms.subscribe(function(){window.localStorage.realms=ko.toJSON(self.realms);});self.realm.subscribe(function(){window.localStorage.realm=self.realm().name;});self.character_name.subscribe(function(){window.localStorage.character_name=self.character_name();});self.character_class.subscribe(function(){window.localStorage.character_class=self.character_class();});self.character_race.subscribe(function(){window.localStorage.character_race=self.character_race();});self.available.subscribe(function(){window.localStorage.available=ko.toJSON(_.map(self.available(),function(c){return c.details;}));});self.completed.subscribe(function(){window.localStorage.completed=ko.toJSON(self.completed());});self.criteria.subscribe(function(){window.localStorage.criteria=ko.toJSON(self.criteria());});self.queue.subscribe(function(){window.localStorage.queue=ko.toJSON(_.map(self.queue(),function(a){return a.id;}));});}
self.locale.subscribe(self.find_realms);self.completed.subscribe(self.update_complete);self.criteria.subscribe(self.update_criteria);};self.load();}
var wowchi=new Wowchi();$(document).ready(function(){ko.applyBindings(wowchi);message.init();});